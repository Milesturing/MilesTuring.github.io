...
2. Because open set is upwards closed, given that y <= sup D, D is a directed set, we have sup D belongs to A. 

3. By the definition of open set again, we have the intersection of D and A is non-empty.

4. Due to the fact A is a subset of {z | x <= z}, we know D intersects {z | x <= z}. 
...
If such a counterexample exists, it would mean the both definitions are not equivalent.
